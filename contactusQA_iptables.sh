#!/bin/bash
CONTACTUS_QA_EIP=54.225.132.166

elb_contactus=dualstack.QA-CONTACT-US-2071663920.us-east-1.elb.amazonaws.com    
elb_contactus1=ipv6.QA-CONTACT-US-2071663920.us-east-1.elb.amazonaws.com
elb_https=QA-Contact-US-HTTPS-450598438.us-east-1.elb.amazonaws.com
elb_https1=dualstack.QA-Contact-US-HTTPS-450598438.us-east-1.elb.amazonaws.com
elb_web=QA-Contact-US-WEB-84683266.us-east-1.elb.amazonaws.com
...
/sbin/iptables -A OUTPUT -p tcp --dport 22 -d ${CONTACTUS_QA_EIP} -j ACCEPT
...
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${CONTACTUS_QA_EIP} -j ACCEPT
...
#ELB Access
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_contactus} -j ACCEPT
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_contactus1} -j ACCEPT
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_https} -j ACCEPT
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_https1} -j ACCEPT
