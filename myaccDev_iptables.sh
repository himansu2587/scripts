#!/bin/bash
MYACCOUNT_DEV_EIP=23.23.95.25

elb_myaccdev=dualstack.dev-myaccount-1843850463.us-east-1.elb.amazonaws.com
...
#22 PORT MYACCOUNT_DEV_EIP
/sbin/iptables -A OUTPUT -p tcp --dport 22 -d ${MYACCOUNT_DEV_EIP} -j ACCEPT
...
#Bangalore Office IPs and MYACCOUNT DEV EIP
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${MYACCOUNT_DEV_EIP} -j ACCEPT
...
#ELB Access (OUTPUT)
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_myaccdev} -j ACCEPT
