#!/bin/bash
PHONE2GO_PUSHKIT1_EIP=54.243.81.197
PHONE2GO_PUSHKIT2_EIP=54.227.240.13
elb_pushkit=dualstack.prod-pushkit-471661970.us-east-1.elb.amazonaws.com
...
#22 PORT PHONE2GO PUSHKIT EIP
/sbin/iptables -A OUTPUT -p tcp --dport 22 -d ${PHONE2GO_PUSHKIT1_EIP} -j ACCEPT
/sbin/iptables -A OUTPUT -p tcp --dport 22 -d ${PHONE2GO_PUSHKIT2_EIP} -j ACCEPT
...
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${PHONE2GO_PUSHKIT1_EIP} -j ACCEPT
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${PHONE2GO_PUSHKIT2_EIP} -j ACCEPT
...
#ELB Access (OUTPUT)
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_pushkit} -j ACCEPT
