#!/bin/bash
CONTACTUS_EIP=54.214.234.14
elb_preprod=dualstack.PREPROD-CONTACT-US-West-1808710813.us-west-2.elb.amazonaws.com
elb_https=dualstack.PreProd-Contact-US-HTTPS-1961473651.us-west-2.elb.amazonaws.com
elb_web=dualstack.PreProd-Contact-US-WEB-HTTPS-2141990651.us-west-2.elb.amazonaws.com
elb_cumgmt=dualstack.cumgmtpreprod-1522082048.us-west-2.elb.amazonaws.com

AWSREPO=repo.us-west-2.amazonaws.com
...
/sbin/iptables -A OUTPUT -p tcp --dport 22 -d ${CONTACTUS_EIP} -j ACCEPT
...
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${CONTACTUS_EIP} -j ACCEPT
...
#ELB Access
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_preprod} -j ACCEPT
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_https} -j ACCEPT
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_web} -j ACCEPT
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_cumgmt} -j ACCEPT
...
/sbin/iptables -A OUTPUT -p tcp --sport 6080 -m state --state ESTABLISHED -j ACCEPT
...
###SFTP PORT to Copy File for Ashish ITIA-64255.

/sbin/iptables -A OUTPUT -p tcp --dport 22 -d 199.19.192.47 -j ACCEPT
