#!/bin/bash
p2g_PROD_PUSHKIT_1=54.243.81.197
p2g_PROD_PUSHKIT_2=54.227.240.13

elb_prod_pushkit=dualstack.prod-pushkit-471661970.us-east-1.elb.amazonaws.com
...
/sbin/iptables -A OUTPUT -p tcp --dport 22 -d ${p2g_PROD_PUSHKIT_1} -j ACCEPT
/sbin/iptables -A OUTPUT -p tcp --dport 22 -d ${p2g_PROD_PUSHKIT_2} -j ACCEPT
...
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${p2g_PROD_PUSHKIT_1} -j ACCEPT
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${p2g_PROD_PUSHKIT_2} -j ACCEPT
