#!/bin/bash 
PHONE2GO_APACHE_SERVER_STAGE_EIP=54.235.87.232

elb_stage_apache=dualstack.stage-apache-elb-266986682.us-east-1.elb.amazonaws.com
...
#22 PORT PHONE2GO_APACHE_SERVER_STAGE_EIP
/sbin/iptables -A OUTPUT -p tcp --dport 22 -d ${PHONE2GO_APACHE_SERVER_STAGE_EIP} -j ACCEPT
...
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${PHONE2GO_APACHE_SERVER_STAGE_EIP} -j ACCEPT
...
#ELB Access (OUTPUT)
/sbin/iptables -A OUTPUT  -m state --state NEW,ESTABLISHED -d ${elb_stage_apache} -j ACCEPT
...
/sbin/iptables -A OUTPUT -p tcp --sport 80 -m state --state ESTABLISHED -j ACCEPT
